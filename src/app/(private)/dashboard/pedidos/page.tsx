      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-2">
        ...
        <div className="flex flex-col gap-3 w-full md:w-auto">
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
                {[
                    { label: 'Todos', val: '' },
                    { label: 'Presupuestos', val: 'PRESUPUESTADO' },
                    { label: 'Confirmados', val: 'CONFIRMADO' },
                    { label: 'Entregados', val: 'ENTREGADO' }
                ].map((btn) => (
                    <a 
                        key={btn.val} 
                        href={btn.val ? `?status=${btn.val}` : `/dashboard/pedidos`} 
                        className={`px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all whitespace-nowrap ${(!statusFilter && btn.val === '') || statusFilter === btn.val ? 'bg-black text-white shadow-lg border-black' : 'bg-white text-gray-400 border-gray-100 hover:text-black'}`}
                    >
                        {btn.label}
                    </a>
                ))}
        ...
        <form action={createOrder} encType="multipart/form-data" className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
            <div className="space-y-1">
                <label className="text-[9px] font-black uppercase text-gray-400 ml-2 italic">Adjuntar Referencias (puedes varias)</label>
            ...
            <div className="space-y-1">
                <label className="text-[9px] font-black uppercase text-gray-400 ml-2 italic">Fecha Entrega (Opcional en Presupuesto)</label>
      ...
      <div className="space-y-10">
                ...
                <div className="space-y-8">
                    ...
                    <div className="bg-black text-white p-6 rounded-[35px] flex items-center justify-between shadow-xl">
                        <div className="flex items-center gap-4">
                            ...
                            <div><p className="text-[10px] font-black uppercase text-zinc-500 tracking-tighter">Item pedido</p><h5 className="text-xl font-black uppercase leading-none truncate max-w-140px md:max-w-none">{product?.name || "Producto"}</h5></div>
                        </div>
                ...
                <div className="flex flex-col justify-between space-y-10">
                    ...
                    <div className="relative">
                        {order.images && order.images.length > 0 ? (
                            ...
                            <div className="h-64 rounded-[40px] bg-gray-50 border-2 border-dashed border-gray-200 flex flex-col items-center justify-center text-gray-300">
                                <ImageIcon size={40} />
                                <p className="text-[10px] font-black uppercase mt-2 italic tracking-widest">Sin imágenes adjuntas</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex justify-between items-end border-b border-gray-100 pb-4 px-2">
                            <div><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Fecha Entrega</p><p className="text-xl font-black uppercase flex items-center gap-2 italic">{order.deliveryDate ? new Date(order.deliveryDate).toLocaleDateString('es-AR', {day:'2-digit', month:'short'}) : "PENDIENTE"}</p></div>
                            <div className="text-right"><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">Total a cobrar</p><p className="text-4xl font-black tracking-tighter">${order.totalPrice.toFixed(2)}</p></div>
                        </div>
                    ...
                    <div className="pt-4 flex flex-col gap-3">
                        <div className="bg-gray-50 p-4 rounded-3xl flex flex-col gap-2">
                            {isQuote ? (
                                ...
                                <div className="text-center py-2 text-xs font-black text-green-600 uppercase flex items-center justify-center gap-2"><CheckCircle2 size={16} /> Trabajo Finalizado</div>
                            )}
                        </div>
        ...
        {orders.length === 0 && (
          <div className="text-center py-24 bg-gray-50 rounded-[60px] border-2 border-dashed border-gray-200 mx-4 md:mx-0">
             ...
             <p className="text-gray-400 font-bold uppercase tracking-widest text-xs italic">Aún no hay operaciones registradas</p>
          </div>
        )}
      </div>
    </div>
