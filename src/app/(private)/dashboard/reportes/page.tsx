...
export default async function ReportsPage() {
  ...
  // 1. OBTENER DATOS (Pedidos confirmados, en proceso o entregados)
  ...
  const dataForExcel = orders.map(o => ({
    ...
    "WhatsApp": o.customerPhone || 'Sin datos',
    "Producto": o.items[0]?.template.name || 'Varios',
    "Cantidad": o.items[0]?.quantity || 0,
    "Facturación ($)": o.totalPrice,
    "Costo Insumos ($)": o.totalCost,
    "Ganancia Neta ($)": o.totalPrice - o.totalCost,
    ...
    <div className="space-y-12 pb-32 pt-8 px-4 max-w-6xl mx-auto">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-1">
            <h1 className="text-[10px] font-black uppercase text-accent tracking-[0.5em] italic">Business Intelligence</h1>
            <h2 className="text-5xl font-black text-black tracking-tighter uppercase leading-none">Analíticas</h2>
        </div>
        
        {/* BOTÓN DE EXCEL INTEGRADO */}
        <ExportExcel 
          data={dataForExcel} 
          fileName={`Reporte-KodaMaker-${monthsNames[now.getMonth()]}-${now.getFullYear()}`} 
          sheetName="Ventas SyG" 
        />
      </header>
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-3 px-4">
            <Calendar className="text-eccent" size={20} />
            <h3 className="font-black uppercase text-sm tracking-widest text-black underline decoration-accent decoration-2 underline-offset-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white p-10 rounded-[45px] border-2 border-zinc-100 shadow-sm flex justify-between items-center group hover:border-black transition-all">
                <div>
                    <p className="text-[11px] font-black uppercase text-zinc-700 tracking-widest mb-2 italic">Facturación Mensual</p>
                    <h4 suppressHydrationWarning className="text-5xl font-black text-black tracking-tighter">${totalRevenueMonth.toLocaleString('es-AR')}</h4>
                </div>
                <ArrowUpRight className="text-zinc-200 group-hover:text-black transition-colors" size={40} />
            </div>
            <div className="bg-green-50 p-10 rounded-[45px] border-2 border-green-200 flex justify-between items-center group hover:bg-green-100 transition-all shadow-sm">
                <div>
                    <p className="text-[11px] font-black uppercase text-green-700 tracking-widest mb-2 italic">Ganancia Neta Mensual</p>
                    <h4 suppressHydrationWarning className="text-5xl font-black text-green-900 tracking-tighter">${totalProfitMonth.toLocaleString('es-AR')}</h4>
                </div>
      ...
      {/* BLOQUE: HISTÓRICO TOTAL ACUMULADO */}
      <section className="space-y-6">
        <div className="flex items-center gap-3 px-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <section className="lg:col-span-8 bg-white p-10 rounded-[50px] shadow-xl border border-gray-50 flex flex-col">
            ...
            <div className="mt-10 pt-8 border-t border-gray-100 overflow-x-auto no-scrollbar">
                <div className="flex justify-between items-center min-w-100 md:min-w-full px-2">
                    {last6Months.map((m) => (
                        <div key={m.name} className="flex-1 text-center group">
                            <p suppressHydrationWarning className="text-[8px] font-black text-zinc-800 uppercase tracking-tighter group-hover:text-accent transition-colors">{m.name}</p>
                ...
                {topProducts.map((p, i) => (
                    <div key={p.name} className="flex items-center justify-between group">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-sm font-black uppercase tracking-tight leading-none text-zinc-100 group-hover:text-accent transition-colors">{p.name}</p>
        ...
        <div className="flex items-center gap-10 relative z-10">
            ...
            <div className="space-y-3 text-center md:text-left">
                ...
                <p className="text-base text-zinc-700 font-medium max-w-lg leading-relaxed">
                    Tu margen de beneficio global es del <span suppressHydrationWarning className="text-black font-black underline decoration-accent decoration-4 underline-offset-8">{globalMargin.toFixed(1)}%</span>.
...
function MiniStat({ label, value, icon, dark = false }: { label: string, value: string, icon: any, dark?: boolean }) {
    return (
        <div className={`p-8 rounded-[40px] border shadow-sm space-y-3 transition-all duration-500 ${dark ? 'bg-zinc-950 border-zinc-900 text-white shadow-zinc-200' : 'bg-white border-zinc-100 text-black'}`}>
            <div className={`flex items-center gap-2 ${dark ? 'text-zinc-600' : 'text-zinc-700'}`}>
                {icon}
                <span className="text-[10px] font-black uppercase tracking-widest">{label}</span>
            </div>
            <p suppressHydrationWarning className="text-3xl font-black tracking-tighter leading-none italic">{value}</p>
        </div>
    )
}