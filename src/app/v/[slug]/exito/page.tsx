...
import { CheckCircle2, MessageSquare, Package, ShoppingBag } from "lucide-react"
...
export default async function SuccessOrderPage({ 
    ...
    // Buscamos el pedido y los datos del dueño (User)
    ...
    // Sacamos el teléfono del DUEÑO (el campo 'phone' que acabamos de agregar a User)
    const sellerPhone = order.user.phone?.replace(/\D/g, '') || "5491100000000" 
    
    const message = encodeURIComponent(
        `¡Hola ${order.user.name}! 👋 Acabo de enviarte una solicitud de pedido por Koda Maker.\n\n` +
    ...
    const whatsappLink = `https://wa.me/${sellerPhone}?text=${message}`
            ...
            <div className="max-w-md w-full space-y-10 text-center">
                ...
                <div className="space-y-4">
                    ...
                    <p className="text-zinc-500 font-medium italic">
                        Gracias {order.customerName}, tu solicitud ha sido enviada con éxito a {order.user.name}.
                    </p>
                </div>

                {/* CARD DE RESUMEN */}
                <div className="bg-white border border-zinc-100 rounded-[40px] p-8 shadow-2xl space-y-6 relative overflow-hidden">
                ...
                <div className="space-y-4 pt-4">
                    ...
                    <Link 
                        href={`/v/${slug}`}
                        className="block text-[10px] font-black text-zinc-400 uppercase hover:text-black transition-colors pt-4"
                    >
                        Volver a la tienda
                    </Link>
                </div>
