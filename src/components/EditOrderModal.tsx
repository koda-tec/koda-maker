            ...
            <div className="fixed inset-0 z-9999 flex items-center justify-center p-4 md:p-10">
                ...
                <div className="bg-white w-full max-w-4xl rounded-[50px] shadow-2xl flex flex-col relative z-10 border border-zinc-200 animate-in slide-in-from-bottom duration-300 max-h-[90vh] overflow-hidden">
                    <div className="p-8 border-b border-zinc-100 flex justify-between items-center bg-white">
                        <h3 className="font-black uppercase text-xl text-black tracking-tighter italic underline decoration-[#f13d4b] decoration-4">Editar Pedido</h3>
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                            <div className="space-y-5">
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase text-zinc-400 ml-2">Nombre Cliente</label>
                                    <input name="customerName" defaultValue={order.customerName} className="w-full p-5 bg-zinc-50 rounded-[25px] outline-none font-black text-black border border-zinc-100 focus:ring-2 focus:ring-[#f13d4b]" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase text-zinc-400 ml-2">WhatsApp</label>
                                    <input name="customerPhone" defaultValue={order.customerPhone} className="w-full p-5 bg-zinc-50 rounded-[25px] outline-none font-black text-black border border-zinc-100 focus:ring-2 focus:ring-[#f13d4b]" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black uppercase text-zinc-400 ml-2">Instrucciones</label>
                                    <textarea name="designDetails" defaultValue={order.designDetails} className="w-full p-5 bg-zinc-50 rounded-[25px] outline-none text-sm h-40 text-black border border-zinc-100 resize-none" />
                                </div>
                            </div>
                            ...
                            <div className="space-y-5">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black uppercase text-zinc-400 ml-2 italic">Subir más fotos</label>
                        ...
                        <div className="border-t border-zinc-100 pt-8">
                            <p className="text-[10px] font-black uppercase text-zinc-400 mb-6 tracking-widest italic">Fotos cargadas (Click para borrar)</p>
                            <div className="grid grid-cols-3 md:grid-cols-5 gap-4">
                                {order.images.map((img: any) => (
                                    <div key={img.id} className="relative aspect-square rounded-[30px] overflow-hidden group border-2 border-zinc-100 shadow-sm">
                                        ...
                                        <button type="button" onClick={() => deleteOrderImage(img.id, img.url)} className="absolute inset-0 bg-red-500/90 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={24}/></button>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="pb-10">
                            <button type="submit" className="w-full p-6 bg-black text-white rounded-[30px] font-black uppercase tracking-[0.2em] shadow-2xl flex items-center justify-center gap-3 hover:bg-[#f13d4b] active:scale-95 transition-all">
